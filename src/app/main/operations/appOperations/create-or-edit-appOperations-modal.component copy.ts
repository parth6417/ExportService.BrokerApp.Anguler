...
import {
    ...
    , AppOperationsAppTransportsLookupTableDto
...
export class BackupCreateOrEditAppOperationsModalComponent extends AppComponentBase implements AfterViewInit {
    ...
    _initWizard(): void {
        ...
        let btnPrev = KTUtil.find(this._wizardEl, '[data-wizard-type="action-prev"]');

        KTUtil.addEvent(btnPrev, 'click', wizard => {
            switch (this._wizardObj.getStep()) {
                case 3: {
                    if (this.appOperations.insuranceService === false && this.appOperations.securityService === true) {
                        this._wizardObj.goTo(3);
                    }
                    else if (this.appOperations.insuranceService === true && this.appOperations.securityService === false) {
                        this._wizardObj.goTo(2);
                    }
                    break;
                }
                case 2: {
            ...
            switch (wizard.getStep()) {
                case 1: {
                    if (this.validateOperationTab1()) {
                        ...
                        if (this.appOperations.insuranceService === false && this.appOperations.securityService === false) {
                            wizard.stop();
                            this.notify.error(this.l('RequiredField', "Servicios incluidos"));
                        }
                        else if (this.appOperations.insuranceService === true && this.appOperations.securityService === false) {
                            wizard.goTo(1);
                        } else if (this.appOperations.insuranceService === false && this.appOperations.securityService === true) {
    ...
    show(appOperationsId?: number, IsDuplicateOperation: boolean = false): void {
        ...
        if (!appOperationsId) {
            ...
            this.appOperations.insuranceService = false;
            this.appOperations.securityService = false;
            ...
            this._appOperationsServiceProxy.getAppOperationsForEdit(appOperationsId).subscribe(result => {
                ...
                console.log(this.appOperations.insuranceService);
    ...
    onInsuredAmountARSChange(): void {
        ...
        //this.appOperations.billAmountUSD = this.appOperations.freightAmountUSD = this.appOperations.dutiesAndChargesUSD = this.appOperations.imaginaryBenefitUSD =0;
        this.appOperations.insuredAmountUSD = round(this.appOperations.insuredAmountARS / this.appOperations.exchangeRate);
    ...
    onInsuredAmountUSDChange(): void {
        ...
        //   this.appOperations.billAmountARS = this.appOperations.freightAmountARS = this.appOperations.dutiesAndChargesARS = this.appOperations.imaginaryBenefitARS = 0;
        this.appOperations.insuredAmountARS = round(this.appOperations.insuredAmountUSD * this.appOperations.exchangeRate);
    };
    ...
    calculateInsurancesTotals(): void {
        ...
        if (this.appOperations.operationState === this.operationState[1]) {
            ...
            this.appOperations.insuranceSubtotalCost = round(this.appOperations.insuredAmountARS * this.appOperations.rateOfSale);


            // var minSalesRate = this.getMinimunSalesRate();
            // if (this.appOperations.rateOfSale < minSalesRate) {
            //     this.appOperations.subtotalInsuranceBilling = round(this.appOperations.insuredAmountARS * minSalesRate);
            // } else {
            //     this.appOperations.subtotalInsuranceBilling =  round(this.appOperations.insuredAmountARS * this.appOperations.rateOfSale);
            // }            
            //this.appOperations.subtotalInsuranceBilling =  round(this.appOperations.insuranceSubtotalCost * this.appOperations.costPercentage);
    ...
    calculateOperationTotals(): void {
        if (this.appOperations.operationState === this.operationState[1]) {
            ...
            this.appOperations.operationTotalARS = round(this.appOperations.insuranceSubtotalCost  + this.securitySubTotal);
            this.appOperations.operationTotalUSD = round(this.appOperations.operationTotalARS / this.appOperations.exchangeRate);
        }
    }
